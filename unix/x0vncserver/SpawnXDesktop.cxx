...
#include <x0vncserver/SpawnXDesktop.h>
...
SpawnXDesktop::SpawnXDesktop(Display* dpy_, Geometry *geometry_)
...
SpawnXDesktop::~SpawnXDesktop() {
  if (running)
    stop();
}


void SpawnXDesktop::poll() {
...
void SpawnXDesktop::start(VNCServer* vs) {
...
void SpawnXDesktop::terminate() {
  kill(getpid(), SIGTERM);
}

bool SpawnXDesktop::isRunning() {
  return running;
}

void SpawnXDesktop::queryConnection(network::Socket* sock,
                               const char* userName)
{
  assert(isRunning());
...
void SpawnXDesktop::pointerEvent(const Point& pos, int buttonMask) {
#ifdef HAVE_XTEST
  if (!haveXtest) return;
...
#ifdef HAVE_XTEST
KeyCode SpawnXDesktop::XkbKeysymToKeycode(Display* dpy, KeySym keysym) {
  XkbDescPtr xkb;
  XkbStateRec state;
  unsigned int mods;
  unsigned keycode;
...
void SpawnXDesktop::keyEvent(rdr::U32 keysym, rdr::U32 xtcode, bool down) {
#ifdef HAVE_XTEST
  int keycode = 0;
...
void SpawnXDesktop::clientCutText(const char* str) {
}

ScreenSet SpawnXDesktop::computeScreenLayout()
{
  ScreenSet layout;
...
unsigned int SpawnXDesktop::setScreenLayout(int fb_width, int fb_height,
...
bool SpawnXDesktop::handleGlobalEvent(XEvent* ev) {
...
void SpawnXDesktop::queryApproved()
{
  assert(isRunning());
...
void SpawnXDesktop::queryRejected()
{
  assert(isRunning());
