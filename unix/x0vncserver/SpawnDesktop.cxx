...
#include <x0vncserver/SpawnDesktop.h>
...
static rfb::LogWriter vlog("SpawnDesktop");

// order is important as it must match RFB extension
static const char * ledNames[SPAWNDESKTOP_N_LEDS] = {
...
//  CharArray dpyStr(displayname.getData());
  ...
  for (int i = 0; i < SPAWNDESKTOP_N_LEDS; i++) {
    Atom a;
    int shift;
    Bool on;
...
void SpawnDesktop::terminate() {
  kill(getpid(), SIGTERM);
}

bool SpawnDesktop::isRunning() {
  return running;
}

void SpawnDesktop::queryConnection(network::Socket* sock,
                               const char* userName)
{
  assert(isRunning());
...
void SpawnDesktop::pointerEvent(const Point& pos, int buttonMask) {
#ifdef HAVE_XTEST
  if (!haveXtest) return;
...
#ifdef HAVE_XTEST
KeyCode SpawnDesktop::XkbKeysymToKeycode(Display* dpy, KeySym keysym) {
  XkbDescPtr xkb;
  XkbStateRec state;
  unsigned int mods;
  unsigned keycode;
...
void SpawnDesktop::keyEvent(rdr::U32 keysym, rdr::U32 xtcode, bool down) {
#ifdef HAVE_XTEST
  int keycode = 0;
...
void SpawnDesktop::clientCutText(const char* str) {
}

ScreenSet SpawnDesktop::computeScreenLayout()
{
  ScreenSet layout;
...
unsigned int SpawnDesktop::setScreenLayout(int fb_width, int fb_height,
...
bool SpawnDesktop::handleGlobalEvent(XEvent* ev) {
...
void SpawnDesktop::queryApproved()
{
  assert(isRunning());
...
void SpawnDesktop::queryRejected()
{
  assert(isRunning());
