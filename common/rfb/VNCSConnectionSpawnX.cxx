...
void VNCSConnectionSpawnX::close(const char* reason)
{
  ...
  // calling code will call VNCServerSpawn's removeSocket() method causing us to
  // be deleted.
  sock->shutdown();
...
void VNCSConnectionSpawnX::pixelBufferChange()
{
  try {
    ...
    if (client.width() && client.height() &&
        (internal_server->getPixelBuffer()->width() != client.width() ||
            internal_server->getPixelBuffer()->height() != client.height()))
    {
      ...
      damagedCursorRegion.assign_intersect(internal_server->getPixelBuffer()->getRect());

      client.setDimensions(internal_server->getPixelBuffer()->width(),
          internal_server->getPixelBuffer()->height(),
          internal_server->getScreenLayout());
      ...
      encodeManager.pruneLosslessRefresh(Region(internal_server->getPixelBuffer()->getRect()));
    }
...
bool VNCSConnectionSpawnX::needRenderedCursor()
{
  ...
  if (!client.supportsLocalCursor())
    return true;
  if (!internal_server->getCursorPos().equals(pointerEventPos) &&
...
void VNCSConnectionSpawnX::authSuccess()
{
  ...
  client.setDimensions(internal_server->getPixelBuffer()->width(),
      internal_server->getPixelBuffer()->height(),
      internal_server->getScreenLayout());
  client.setName(internal_server->getName());
  client.setLEDState(internal_server->getLEDState());
  
  // - Set the default pixel format
  client.setPF(internal_server->getPixelBuffer()->getPF());
  char buffer[256];
...
void VNCSConnectionSpawnX::pointerEvent(const Point& pos, int buttonMask)
{
  ...
  pointerEventPos = pos;
  internal_server->pointerEvent(this, pointerEventPos, buttonMask);
}
...
void VNCSConnectionSpawnX::keyEvent(rdr::U32 keysym, rdr::U32 keycode, bool down) {
  ...
  if ((internal_server->getLEDState() == ledUnknown) &&
      ((keysym == XK_Caps_Lock) ||
       (keysym == XK_Num_Lock) ||
  ...
  if (!client.supportsLEDState()) {
    ...
    if (down && (internal_server->getLEDState() != ledUnknown)) {
      ...
      if (((keysym >= XK_A) && (keysym <= XK_Z)) ||
        ...
        lock = internal_server->getLEDState() & ledCapsLock;

        if (lock == (uppercase == shift)) {
          vlog.debug("Inserting fake CapsLock to get in sync with client");
          internal_server->keyEvent(XK_Caps_Lock, 0, true);
          internal_server->keyEvent(XK_Caps_Lock, 0, false);
        }
      }
      ...
      if (((keysym >= XK_KP_Home) && (keysym <= XK_KP_Delete)) ||
        ...
        number = ((keysym >= XK_KP_0) && (keysym <= XK_KP_9)) ||
                  (keysym == XK_KP_Separator) || (keysym == XK_KP_Decimal);
        shift = isShiftPressed();
        lock = internal_server->getLEDState() & ledNumLock;
        ...
        if (shift) {
          ...
          internal_server->keyEvent(XK_Num_Lock, 0, true);
          internal_server->keyEvent(XK_Num_Lock, 0, false);
        }
      }
    }
  }
  ...
  VNCSConnectionSpawnXShiftPresser shiftPresser(internal_server);
                                      ...
                                      const ScreenSet& layout)
{
  ...
  result = internal_server->setDesktopSize(this, fb_width, fb_height, layout);
...
void VNCSConnectionSpawnX::handleClipboardData(const char* data)
{
  if (!accessCheck(AccessCutText)) return;
  if (!rfb::Server::acceptCutText) return;
  internal_server->handleClipboardData(this, data);
}
...
void VNCSConnectionSpawnX::writeDataUpdate()
{
  ...
  if (!ui.copied.is_empty() && !damagedCursorRegion.is_empty()) {
    ...
    bogusCopiedCursor.assign_intersect(internal_server->getPixelBuffer()->getRect());
  ...
  if (updateRenderedCursor) {
    updates.add_changed(internal_server->getRenderedCursor()->getEffectiveRect());
    needNewUpdateInfo = true;
  ...
  if (!internal_server->getPendingRegion().is_empty()) {
    req.clear();
    ui.changed.clear();
    ui.copied.clear();
  }
  ...
  if (needRenderedCursor()) {
    ...
    cursor = internal_server->getRenderedCursor();
  ...
  encodeManager.writeUpdate(ui, internal_server->getPixelBuffer(), cursor);
...
void VNCSConnectionSpawnX::writeLosslessRefresh()
{
  ...
  pending = internal_server->getPendingRegion();
  if (!pending.is_empty()) {
    UpdateInfo ui;
  ...
  nextUpdate = internal_server->msToNextUpdate();
  ...
  encodeManager.writeLosslessRefresh(req, internal_server->getPixelBuffer(),
...
void VNCSConnectionSpawnX::setCursor()
{
  ...
  if (needRenderedCursor()) {
    ...
    client.setCursor(*internal_server->getCursor());
